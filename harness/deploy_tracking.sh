...
cp "$REPO/harness/vio_live.py" "$TMP_STAGE/vio_live.py"
...
cp -a /home/luke/harness/vio_live.py "$backup/vio_live.py"
was_active=0
...
restore() {
  ...
  cp -a "$backup/vio_live.py" /home/luke/harness/vio_live.py
  if [ "$was_active" = 1 ]; then systemctl restart vio@luke; fi
}
trap restore ERR
install -o luke -g luke -m 755 vio_live /home/luke/vio_live/vio_live
install -o luke -g luke -m 644 vio_live.py /home/luke/harness/vio_live.py
...
sha256sum /home/luke/vio_live/vio_live /home/luke/harness/vio_live.py
