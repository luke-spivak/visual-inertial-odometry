...
#
#   harness/vio_live/build.sh          build, bundle, copy to viopi:~/vio_live, gate
#
...
docker image inspect "$IMAGE" >/dev/null 2>&1 || "$REPO/harness/buildenv/buildenv.sh" build
...
cmake -S harness/vio_live -B \$B/live -G Ninja -DCMAKE_BUILD_TYPE=Release \
...
cp "$REPO"/openvins/hw_pi/*.yaml "$REPO/$B/bundle/config/"
...
scp -q "$REPO/harness/vio_live.py" "$REPO/harness/imu_log.py" "$PI:harness/"
