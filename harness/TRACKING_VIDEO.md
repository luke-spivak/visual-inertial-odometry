...
On the Mac, create a Python environment and install `numpy` and `opencv-python`.
```sh
...
Install FFmpeg separately and ensure `ffmpeg` is on PATH. The fetch command also
requires `ssh` and `rsync` with access to `viopi`.
...
```sh
python harness/fetch_tracking.py datasets/flights
```
...
```sh
python harness/render_tracking.py datasets/flights/run-YYYYMMDD-HHMMSS
```
...
The standalone C++ test `harness/vio_live/test_feature_log.cpp` runs in Linux
