...
Reads <prefix>.y16 + <prefix>.pts, writes <prefix>_frames/cam0/<ns>.png, builds
...
it in this repo:
  * a byte count that is not whole 640x400 16-bit frames means the wrong mode
...
def main():
    ...
    with open(pts) as fh:
        for line in fh:
            ...
            if line and not line.startswith("#"):
                ts.append(int(round(float(line) * 1e6)))      # ms -> ns
    ts = np.array(ts, dtype=np.int64)
    if len(ts) != len(mono):
        fail(f"{len(mono)} frames but {len(ts)} timestamps -- they must pair one to one")
    d = np.diff(ts)
    ...
    print(f"  brightness  mean {mono.mean():.1f} / 255")
