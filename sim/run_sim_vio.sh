...
echo "==> gazebo ($WORLD)"
GZRES="$HERE/gazebo/models:$HERE/gazebo/worlds:$HOME/ardupilot_gazebo/models:$HOME/ardupilot_gazebo/worlds"
nohup env -i \
...
nohup ros2 run ros_gz_bridge parameter_bridge \
  --ros-args -p config_file:="$HERE/gazebo/ros_gz_bridge_vio.yaml" \
  > "$RUN/bridge.log" 2>&1 &
own $!
sleep 8
...
if grep -qE "UNTRACKABLE|LOOKS BLACK|NO IMAGES" "$RUN/camera_check.log" 2>/dev/null; then
  ...
  echo "      gz sim -s -r -v4 render_probe.sdf   (see sim/gazebo/worlds/)" >&2
...
nohup ros2 launch ov_msckf subscribe.launch.py \
  config_path:="$HERE/config/estimator_config.yaml" \
...
# identical input removes the run-to-run timing variance that docs/development-log.md
