...
void near(const Eigen::Vector3d& actual, const Eigen::Vector3d& expected, const std::string& name,
          double tolerance = 1e-8) {
    require(actual.allFinite() && (actual - expected).norm() < tolerance, name);
}

template <typename Function> void rejects(Function operation) {
    bool rejected = false;
    try {
        operation();
...
Eigen::Matrix3d calibration() {
    ...
    rotation << -0.99991866, -0.01269620, 0.00121422, -0.01266913, 0.99971620, 0.02017451,
...
void reference_cases(const std::string& path) {
    ...
    while (std::getline(file, line)) {
        ...
        double tilt, x, y, z, w;
        bool upside_down;
        vio::EstimatorEstimate estimate;
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            require(std::abs(std::atan2(std::sin(error), std::cos(error))) < 1e-8,
                    "attitude parity");
        }
        ...
        near(transform.transform(estimate).roll_pitch_yaw_rad, navigation.roll_pitch_yaw_rad,
             "quaternion sign and scale");
        ++count;
    }
...
void validity_cases() {
    ...
    auto check = [&](EstimateRejection expected, long age = 500) {
        require(vio::validate_estimate(estimate, {3}, MonotonicTime{1500}, MonotonicTime{age}) ==
                    expected,
