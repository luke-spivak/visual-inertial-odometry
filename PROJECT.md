...
**It is not the only rename.** Checked against parameter metadata generated
from this exact tree (`Tools/autotest/param_metadata/param_parse.py --vehicle
ArduCopter`, 5,771 parameters), these no longer exist under their old names:

| old | this tree | note |
|---|---|---|
| `ATC_ACCEL_R/P/Y_MAX` | `ATC_ACC_R/P/Y_MAX` | **deg/s², not cdeg/s²** — an old-guide value is 100× too large |
| `ANGLE_MAX` | `ATC_ANGLE_MAX` | deg |
| `ACRO_YAW_P` | `ACRO_Y_RATE` | deg/s |
| `LAND_SPEED` | `LAND_SPD_MS` | m/s |

Mission Planner's initial-parameter calculator and most forum advice still write
the old names. **ExpressLRS's own ArduPilot guide is wrong for this tree too:** it
gives `RC_OPTIONS` bit 10 for 420 kbaud, which here is "multiple receiver
support"; 420 kbaud is **bit 13** (`AP_RCProtocol_CRSF.h:55`). And the binary has
no GyroFFT (`!HAL_GYROFFT_ENABLED`, from `extract_features.py` on the flashed
...
**The board README disagrees with its own hwdef in two places.** It calls UART1
DMA-enabled and gives battery defaults `BATT_VOLT_MULT 11.2`, `BATT_AMP_PERVLT
52.7`. The hwdef the firmware was built from has UART1 `NODMA` and scales 11.0
...
**Recovery confirmed.** After a full power cycle with the clips reseated and
taped: one-shot |accel| 9.58 m/s², and `imu_irq_check.sh` streams 6,580 /
6,584 samples per 15 s at watermark 64 / 8. Watermark 8 was never the problem.

**Bench run 3 (2026-09-10): first live VIO on the Pi.** 60 s handheld at
~25 lux (4 ms, gain 9.7), started, rested mid-run, and ended on the same table
spot.

| | |
|---|---|
| frames | 1,151 received, 1,151 processed, 0 dropped |
| update, per frame | 22 ms avg, 54 ms max (frame interval 52 ms) |
| lag behind real time | 25–56 ms |
| IMU | 438–441 Hz for the whole run, kernel buffer ≤ 6 samples |
| path | 23.7 m, max 1.6 m/s, extent 2.2 × 1.9 × 1.2 m |
| start → mid rest (0–32 s, ~14 m) | **0.80 m** (0.73 horizontal, 0.35 vertical) |
| mid rest → end (35–58 s, ~9 m) | **0.02 m** |
| closure, start → end | **0.77 m = 3.25 % of path** |

All the error accrued in the first half, and the second half closed to 2 cm:
early convergence, not steady drift. The first attempt at this run never
initialised. Held in the hand, the rig never went below 15 °/s (~290 px/s at
...
**ZUPT fixes it.** Same recording, three replays per config:

| config | runs | diverged | closure |
|---|---|---|---|
| as walked (ZUPT dead) | 4 | 2 | 0.77 %, 1.14 %, 331 m, 6.1 km |
| **ZUPT enabled** | 3 | 0 | **0.18, 0.19, 0.31 %** (vertical −0.06 to −0.09 m) |
| ZUPT + `init_imu_thresh` 1.5 | 3 | 0 | 0.22, 0.25, 0.33 % |

With ZUPT (`zupt_max_disparity: 1.0` px, `zupt_only_at_beginning: false`)
all six runs initialise on the same frame. There is no lift-off jump: height
now rises the ~0.3 m from table to hand, and ZUPT fired 374 times in run 1. The
jolt threshold adds nothing, so it stays at 0.5 and only ZUPT changed. It is
now in `openvins/hw_pi`.

**The resting view mattered too.** The rig sat on the table looking at a
mostly white surface above a dark, distant rack. At lift-off the white left the
frame and mean brightness halved (50 → 23), so most tracks broke at once.
Rest facing the room. ZUPT stays regardless: a drone on the pad with a 30°
nose-down camera has the same kind of poor resting view.

**Config gotcha, caught by the Pi parse check.** An explanation written as a
trailing comment after a plain value (`key: false # NOTE: …`) makes OpenCV's
YAML parser read the colon as a new key and abort with "Incorrect
indentation". Explanations go on their own comment lines.

**walk2 (2026-09-10): "not initialising" was ZUPT holding it, and my
indicator was wrong.** With ZUPT on, OpenVINS initialised within seconds on
the table (replay: disparity 0.2 / 0.9 px, "successful initialization"). Every
frame at rest then took the zero-velocity update, which returns before the full
update (VioManager.cpp:299–304). `initialized()` needs a full update to have
run (`timelastupdate`, set only at :651), so it stays false for as long as the
rig is still. `vio_live` printed INITIALIZED off `initialized()` and the
protocol said to wait for it, so the rig sat 16 s initialised, waiting to be
lifted. `vio_live` now announces OpenVINS's own init (`initialized_time()`)
with "holding still, lift it and walk", and runs OpenVINS at INFO until then
so a stuck init shows its reason live.

The run also exposed a shutdown bug. A second Ctrl-C SIGKILLed `vio_live`
before it wrote metadata, leaving 362 frames against a stale 236-record file
from an earlier attempt. Now the launcher ignores further Ctrl-C while
stopping, `vio_live` writes metadata frame by frame, and the converter accepts
a cut-short recording, dropping at most a partial frame and a few records.

### Step 7 result: walk3 with ZUPT (2026-09-10)

Same loop, fixed config. The rig rested facing the room and was lifted when
`vio_live` said INITIALIZED. It initialised on the table, ZUPT held it for
4.3 s, and the first full update came at the lift. ~17 lux (4 ms, gain 12.7).
Estimate and plot: `results/vio_walk3_2026-09-10-*`.

| walk3 | first update (s) | closure | horizontal | vertical |
|---|---|---|---|---|
| **live, Pi** | 6030.81 | **0.54 %** | 0.30 m | −0.08 m |
| offline z1 | 6030.82 | 0.73 % | 0.41 m | −0.10 m |
| offline z2 | 6030.82 | 0.69 % | 0.39 m | −0.06 m |
| offline z3 | 6030.82 | 0.61 % | 0.35 m | −0.01 m |
| offline z4 | 6030.82 | 0.73 % | 0.40 m | −0.10 m |

56.9 m in 97 s, 1,957 frames, 0 dropped, 25 ms per update. **Closure 0.54–0.73 %
over five runs (median 0.69 %), none diverged.** Every run takes its first
update on the same frame. On walk1 the old config spread from 0.7 % to a
6 km divergence.

Height is fixed: the lift shows (median +0.22 m while carried) and the end
rest is within 0.1 m. What is left is horizontal, 0.30–0.41 m. That is larger
than walk1's 0.15 m, but that was a different walk at a third of the light.
Nothing here isolates a cause. More light is the first lever: this ran at gain
12.7, and bench 1 showed what noise does to tracks.

**Step 7 gate met: VIO produces a sane, repeatable trajectory on real data,
live on the Pi, at 0.54–0.73 % closure over 57 m.**
