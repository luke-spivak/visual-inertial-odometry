...
cp "$REPO/src/capture_session.py" "$REPO/src/cli.py" "$REPO/src/flight_config.py" "$REPO/src/imu_device.py" "$TMP_STAGE/"
...
cp -a /home/luke/src/capture_session.py /home/luke/src/cli.py /home/luke/src/flight_config.py /home/luke/src/imu_device.py "$backup/"
was_active=0
...
restore() {
  ...
  cp -a "$backup/capture_session.py" "$backup/cli.py" "$backup/flight_config.py" "$backup/imu_device.py" /home/luke/src/
  if [ "$was_active" = 1 ]; then systemctl restart vio@luke; fi
}
trap restore ERR
install -o luke -g luke -m 755 vio_live /home/luke/vio_live/vio_live
install -o luke -g luke -m 644 capture_session.py cli.py flight_config.py imu_device.py /home/luke/src/
...
sha256sum /home/luke/vio_live/vio_live /home/luke/src/capture_session.py /home/luke/src/cli.py /home/luke/src/flight_config.py /home/luke/src/imu_device.py
journalctl -u vio@luke -n 12 --no-pager
INSTALL
(cd "$TMP_STAGE" && shasum -a 256 vio_live capture_session.py cli.py flight_config.py imu_device.py install.sh > SHA256SUMS)
ssh viopi 'mkdir -p /home/luke/vio-tracking-staged'
scp "$TMP_STAGE/vio_live" "$TMP_STAGE/capture_session.py" "$TMP_STAGE/cli.py" "$TMP_STAGE/flight_config.py" "$TMP_STAGE/imu_device.py" "$TMP_STAGE/install.sh" "$TMP_STAGE/SHA256SUMS" viopi:/home/luke/vio-tracking-staged/
