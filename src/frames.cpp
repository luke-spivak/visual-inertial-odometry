...
/**
 * Convert the estimator's z-up world axes to local NED conventions:
 * (x, y, z) -> (y, x, -z). Swapping x/y preserves handedness when z is flipped.
 * This does not establish geographic north; heading alignment is separate.
 */
...
/**
 * Rotate camera optical coordinates (right/down/forward) into aircraft body
 * coordinates (forward/right/down), accounting for downward tilt and inversion.
 * With zero tilt and an upright camera: body (x, y, z) = camera (z, x, y).
 */
...
FrameTransform::FrameTransform(const Eigen::Matrix3d& camera_from_imu, CameraMount mounting) {
    if (!camera_from_imu.allFinite() || !std::isfinite(mounting.tilt_below_horizontal_rad) ||
        (camera_from_imu * camera_from_imu.transpose() - Eigen::Matrix3d::Identity()).norm() >
            1e-6 ||
    ...
    // Compose right to left: IMU -> camera -> aircraft body.
    body_from_imu_ = body_from_camera(mounting) * camera_from_imu;
}

ArduPilotMeasurement FrameTransform::transform(const EstimatorEstimate& estimate) const {
    ...
    // Compose right to left: body -> IMU -> world -> local NED.
    // A rotation's transpose is its inverse, hence body_from_imu_.transpose().
    const Eigen::Matrix3d ned_from_body =
        ned_from_world() * orientation.toRotationMatrix() * body_from_imu_.transpose();
    // Extract ZYX Euler angles, returned as roll/pitch/yaw. Clamp asin's input
    // against roundoff; roll/yaw are not unique at pitch = +/- pi/2.
    ...
    return {estimate.timestamp, estimate.generation, ned_from_world() * estimate.position_world_m,
